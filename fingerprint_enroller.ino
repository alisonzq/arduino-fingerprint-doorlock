...
void loop() 
{
  ...
  Serial.println("Please type -2 to delete a specific fingerprint");
  Serial.println("Please type in the ID # (from 1 to 127) you want to save the finger as...");
  id = readnumber();

  if (id == 0) { 
    return;
  } else if (id ==  uint8_t(-1)) {
    Serial.print("Cleared all fingerprints from database");
    finger.emptyDatabase();
    return;    
  } 

  //CODE AJOUTÉ
  else if (id ==  uint8_t(-2)) {
...
//source : https://github.com/un0038998/FingerPrint_Lock_System/blob/main/FingerPrint_Enroll/FingerPrint_Enroll.ino
